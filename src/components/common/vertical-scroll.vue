    ...
    <div>
      ...
      :style="`margin-top:${dragTip.translate}px`">
        <div class="clear" v-if="dragTip.showLoding">
          ...
          <div class="loading-down-txt">{{dragTip.text}}</div>
        </div>
      </div>
      <slot></slot>
      <div v-if="pullup" class="pullup">
        <div class="clear" v-if="!isDone">
          ...
          <div class="loading-up-txt">正在加载中.....</div>
        </div>
...
<style scoped lang="less" type="stylesheet/stylus">
.scroll-wrap{height:100%;overflow:hidden
}
/* 下拉刷新 */
.pulldown,.pullup{width:100%;height:50px;position:relative;padding:.1rem;
    div.clear{
        position:absolute;
        left:50%;
      ...
      .loading-down-txt,.loading-up-txt{color:#999}
      .loading-up-txt,.loading-up-img{display: inline-block}
    }
.list-donetip{text-align:center;line-height:50px;font-size:.28rem}
</style>